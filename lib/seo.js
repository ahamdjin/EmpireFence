import { getAreaDirectory } from "@/lib/content";
...
function buildAreaServedSchema() {
  const areaDirectory = getAreaDirectory();
  const areaTitles = areaDirectory.length
    ? areaDirectory.map((area) => area.data.title)
    : serviceAreas.map((area) => area.title);

  return [
    { "@type": "City", name: business.city },
    ...areaTitles.map((title) => ({ "@type": "City", name: title })),
  ];
}
...
export function buildPageMetadata({
  ...
  const includesBrand =
    title.includes(business.shortName) || title.includes(business.name);
  const fullTitle = includesBrand ? title : `${title} | ${business.shortName}`;
