...
const pagesContentPath = path.join(contentRoot, "pages");
...
export async function getAllAreas() {
  ...
  return rendered.sort(sortByTitle);
}
...
export function getAreaDirectory() {
  return readDirectory(areasContentPath, "content/areas", {
    requiredFields: ["title", "summary", "heroImage", "faqs"],
  }).sort(sortByTitle);
}

export async function getAllPages() {
  const pages = readDirectory(pagesContentPath, "content/pages", {
  ...
  return Promise.all(
    pages.map(async (page) => ({
      ...page,
      html: await toHtml(page.content),
      sections: await renderSections(page.content),
    })),
  );
}
